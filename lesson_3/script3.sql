...
DROP TABLE IF EXISTS `present_photo`;
CREATE  TABLE `present_photo` (
	`photo_id` SERIAL,
...
DROP TABLE IF EXISTS `present_send`;
CREATE TABLE `present_send` (
	`senler_id` BIGINT UNSIGNED NOT NULL,
	`accept_id` BIGINT UNSIGNED NOT NULL,
	`present_id` BIGINT UNSIGNED NOT NULL,
	
	FOREIGN KEY (senler_id) REFERENCES users(id),
